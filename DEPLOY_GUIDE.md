...
4. Buka **GitHub** Anda, klik tombol **New** untuk membuat repositori baru:
   ...
   * Jangan centang *"Add a README file"*, *"Add .gitignore"*, atau *"Choose a license"* karena kita sudah memilikinya secara lokal.
...
5. Salin tautan repositori GitHub Anda, lalu jalankan perintah berikut di terminal komputer Anda untuk menghubungkannya:
   ...
   git remote add origin https://github.com/USERNAME/masandigital.git
...
5. Buka file `supabase.sql` di komputer Anda, salin seluruh isi kodenya, lalu tempel (*paste*) di SQL Editor Supabase.
6. Klik tombol **Run** (tombol berwarna hijau di kanan atas) untuk mengeksekusi script SQL. 
   * *Status sukses*: Skema tabel `articles`, `settings`, artikel demo awal, serta seluruh aturan **Row Level Security (RLS)** yang ketat akan langsung terpasang di database cloud Anda.
...
6. Sebelum mengklik Deploy, gulir ke bawah ke bagian **Environment Variables** (Variabel Lingkungan) dan klik **Add Variable** (ini adalah langkah kritikal agar website bisa terhubung dengan Supabase):
   * Masukkan variable pertama:
     ...
     * **Value**: *(Tempelkan URL Supabase yang Anda salin pada Langkah 2 poin 8)*
   * Masukkan variable kedua:
     * **Key**: `NEXT_PUBLIC_SUPABASE_ANON_KEY`
     * **Value**: *(Tempelkan Anon Key Supabase yang Anda salin pada Langkah 2 poin 8)*
...
9. Setelah selesai (*Site is live*), Netlify akan memberikan URL acak gratis (contoh: `https://masandigital.netlify.app`). Klik tautan tersebut untuk menguji website Anda yang sudah mengudara!
