  ...
  useEffect(() => {
    async function loadPrivacy() {
      try {
        ...
        setPrivacyContent(settings.privacy_content);
      } catch (err) {
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-3xl p-8 md:p-12 shadow-sm space-y-6">
            <div className="flex items-center gap-3 pb-6 border-b border-outline-variant/10">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-extrabold text-on-surface tracking-tight">Privacy Policy</h1>
                <p className="text-xs text-on-surface-variant mt-1">Official Privacy Statement</p>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <section className="space-y-4 text-sm text-on-surface-variant leading-relaxed whitespace-pre-wrap">
                {privacyContent || 'Privacy Policy is currently empty. Please configure it in your Admin settings.'}
              </section>
            )}
          </div>
        </div>
